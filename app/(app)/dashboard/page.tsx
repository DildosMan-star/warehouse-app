...
export default async function DashboardPage() {
  ...
  const [{ count: productCount }, { data: stockRows }, { count: movementsToday }, { data: recentMovements }] =
    await Promise.all([
  ...
  const totalUnits = (stockRows ?? []).reduce((sum, row: any) => sum + Number(row.quantity), 0);

  const byProduct = new Map<string, { name: string; sku: string; qty: number; reorder: number }>();
  for (const row of (stockRows ?? []) as any[]) {
    ...
    const existing = byProduct.get(key) ?? {
      ...
      qty: 0,
      reorder: row.products?.reorder_point ?? 0,
    };
    existing.qty += Number(row.quantity);
    byProduct.set(key, existing);
  }
  const lowStock = [...byProduct.values()].filter((p) => p.qty <= p.reorder);
  ...
  return (
    <div>
      ...
      <div className="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <KpiCard label="จำนวนหน่วยคงคลังรวม" value={totalUnits.toLocaleString()} />
        <KpiCard
          label="สินค้าใกล้หมด / หมด"
          value={lowStock.length}
          tone={lowStock.length > 0 ? "danger" : "good"}
        />
        <KpiCard label="รายการเคลื่อนไหววันนี้" value={movementsToday ?? 0} />
      </div>

      <div className="grid grid-cols-1 gap-6 lg:grid-cols-5">
        <section className="card p-5 lg:col-span-2">
          <h2 className="mb-4 text-lg font-semibold">สินค้าใกล้ถึงจุดสั่งซื้อ</h2>
          {lowStock.length === 0 ? (
            <p className="text-sm text-ink/50">ยังไม่มีสินค้าที่ต่ำกว่าจุดสั่งซื้อ</p>
          ) : (
              ...
              {lowStock.map((p) => (
                ...
                >
                  <div>
                    ...
                    <div className="font-mono text-xs text-ink/50">{p.sku}</div>
                  </div>
        ...
        <section className="card p-5 lg:col-span-3">
          <h2 className="mb-4 text-lg font-semibold">รายการเคลื่อนไหวล่าสุด</h2>
